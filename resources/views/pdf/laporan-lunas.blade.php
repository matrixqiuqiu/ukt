...
<tbody>
@forelse($data as $i=>$p)
<tr><td style="color:#000;">{{ $i+1 }}</td><td style="font-family:monospace;color:#000;">{{ $p->tagihan?->mahasiswa?->nim ?? '-' }}</td><td>{{ $p->tagihan?->mahasiswa?->nama_lengkap ?? '-' }}</td><td>{{ $p->tagihan?->mahasiswa?->jurusan ?? '-' }}</td><td style="text-align:center">{{ $p->tagihan?->mahasiswa?->angkatan ?? '-' }}</td><td>{{ $p->tagihan?->tahun_akademik ?? '-' }}</td><td style="text-align:center">{{ $p->tagihan?->semester ?? '-' }}</td><td style="text-align:right">Rp {{ number_format($p->jumlah_bayar,0,',','.') }}</td><td>{{ $p->verified_at?->format('d/m/Y H:i') ?? $p->updated_at?->format('d/m/Y H:i') }}</td><td style="color:#000;">@if($p->_beasiswa) <span style="color:#000;font-weight:700;">{{ $p->_beasiswa }}</span><br><span style="color:#000;font-size:7px;">{{ $p->metodePembayaran?->nama_metode ?? '' }}</span> @else {{ $p->metodePembayaran?->nama_metode ?? '-' }} @endif</td><td style="font-family:monospace;font-size:7px">{{ $p->va_number ?? '-' }}</td></tr>
